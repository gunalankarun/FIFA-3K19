...
loop	LDR R1, =SSI0_SR_R		
		...
		LDR R1, =SSI0_DR_R
		MOV R2, R0				; write command to SSI0_DR_R
		STR R2, [R1]
...
loop2	LDR R1, =SSI0_SR_R
		...
		LDR R2, [R1]			; read SSI0_SR_R
		AND R2, R2, #0x02
		CMP R2, #0x02			; check if bit 1 is low
		BNE loop2				; loop back if yes
		
		LDR R1, =DC			
		MOV R2, #DC_DATA		; set DC to one
		STR R2, [R1]
		
		LDR R1, = SSI0_DR_R
		MOV R2, R0				; write data to SSI0_DR_R
		STR R2, [R1]	
